...
function paperReadingStatus(p){ const raw=String(p.reading_status||p.status||'New').trim().toLowerCase(); if(['read','done','finished'].includes(raw)) return 'Read'; if(['reading','in progress'].includes(raw)) return 'Reading'; if(raw==='archived') return 'Archived'; return 'To Read'; }
function readingStatusKey(p){ return paperReadingStatus(p).toLowerCase().replace(/\s+/g,'-'); }
function readingStatusBadge(p){ const st=paperReadingStatus(p); return `<em class="read-status read-${esc(readingStatusKey(p))}">${esc(st)}</em>`; }
function readingStatusAction(p){ const isRead=paperReadingStatus(p)==='Read'; return {label:isRead?'Mark unread':'Mark read', next:isRead?'To Read':'Read'}; }
...
function card(p, cls='paper-card'){ return `<a class="${cls}" href="${aiHref(p)}"><div class="card-meta"><span>${esc(p.topic_slug||'paper')}</span><span>${esc(metaLine(p))}</span></div><h3>${esc(p.title)}</h3><p>${esc(short(summaryOf(p), cls==='small-card'?150:210))}</p><div class="tag-row">${readingStatusBadge(p)}${(p.tags||[]).slice(0,4).map(t=>`<em>${esc(t)}</em>`).join('')}</div></a>`; }
...
function filteredPapers(prefix=''){ const q=($(`#${prefix}paper-search`)?.value || $('#search')?.value || '').toLowerCase(); const topic=$(`#${prefix}topic-filter`)?.value || $('#topic-filter')?.value || ''; const source=sourceKey($(`#${prefix}source-filter`)?.value || ''); const tag=$(`#${prefix}tag-filter`)?.value || $('#tag-filter')?.value || ''; const reading=$(`#${prefix}reading-status-filter`)?.value || $('#reading-status-filter')?.value || ''; const status=prefix==='ai-'?state.statusFilter:''; return state.papers.filter(p=>{ const tags=Array.isArray(p.tags)?p.tags:[]; const projects=paperProjects(p); const text=`${p.title} ${summaryOf(p)} ${p.abstract||''} ${tags.join(' ')} ${projects.join(' ')} ${paperReadingStatus(p)} ${p.venue||''} ${p.source_label||''}`.toLowerCase(); const taxonomyOk=prefix!=='ai-' || state.taxonomyMode==='all' || (state.taxonomyMode==='project'&&projects.includes(state.taxonomyValue)) || (state.taxonomyMode==='topic'&&p.topic_slug===state.taxonomyValue) || (state.taxonomyMode==='venue'&&paperSourceKeys(p).includes(sourceKey(state.taxonomyValue))) || state.taxonomyMode==='status'; const readingOk=!reading || readingStatusKey(p)===reading; const statusOk=!status || (status==='pdf'&&(p.local_document||p.source_url)) || (status==='note'&&summaryOf(p)!=='待补充。') || (status==='todo'&&summaryOf(p)==='待补充。'); return taxonomyOk && readingOk && (!q||text.includes(q)) && (!topic||p.topic_slug===topic) && (!source||paperSourceKeys(p).includes(source)) && (!tag||tags.includes(tag)) && statusOk; }); }
...
function selectPaper(p){ state.paper=p; state.lastNoteDraft=null; state.selectedVisual=null; const noteEditor=$('#reading-note-editor'); if(noteEditor) noteEditor.value=''; const url=new URL(location.href); url.searchParams.set('id',p.paper_id); history.replaceState(null,'',url); const statusAction=readingStatusAction(p); $('#ai-paper-header').innerHTML=`<p class="eyebrow">${esc(p.topic_slug||'paper')} · ${esc(p.display_venue||p.venue||p.source_label||'')}</p><h1>${esc(p.title)}</h1><p>${esc((p.authors||[]).join(', '))}</p><div class="tag-row">${readingStatusBadge(p)}${(p.tags||[]).slice(0,8).map(t=>`<em>${esc(t)}</em>`).join('')}<button class="tiny mark-read-btn" data-mark-read data-next-reading-status="${esc(statusAction.next)}" type="button">${esc(statusAction.label)}</button><span class="reader-status-notice" data-reader-status-notice></span></div>`; $('#ai-paper-header [data-mark-read]')?.addEventListener('click',e=>updateReadingStatus(e.currentTarget.dataset.nextReadingStatus||'Read')); const pdf=pdfUrl(p); loadPdfPreview(p); $('#open-source').href=p.source_url||pdf||'#'; $('#pdf-status').textContent=p.local_document?'local synced PDF':(pdf?'rendered same-origin PDF':'no PDF found'); $('#synced-note').innerHTML=noteHtml(p); renderFigureGallery(p); renderAudioPanel(p); const tldr=$('#insight-tldr'); if(tldr) tldr.textContent=short(summaryOf(p),260); const score=$('#insight-score'); if(score) score.textContent=scoreFor(p); const nov=$('#insight-novelty'); if(nov) nov.textContent=noveltyFor(p); const meta=$('#insight-meta'); if(meta) meta.innerHTML=`<span>${esc(metaLine(p)||'No venue')}</span><span>${esc((p.tags||[]).slice(0,2).join(' · ')||p.topic_slug||'paper')}</span>`; $$('.ai-paper-item').forEach(el=>el.classList.toggle('active',el.dataset.id===p.paper_id)); }
function renderAiList(){ renderTaxonomy(); }
function selectInitialPaper(){ const id=new URLSearchParams(location.search).get('id'); const p=state.papers.find(x=>x.paper_id===id)||state.papers[0]; if(p) selectPaper(p); }
...
function setReaderStatusNotice(text,kind='info'){ const box=$('[data-reader-status-notice]'); if(!box) return; box.textContent=text||''; box.className=`reader-status-notice ${kind}`; }
async function updateReadingStatus(status){ if(!state.paper){ setReaderStatusNotice('先选择一篇论文。','warn'); return; } const btn=$('[data-mark-read]'); if(btn) btn.disabled=true; setReaderStatusNotice('Saving status…','info'); try{ const res=await fetch('/api/reading-status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({paper_key:state.paper.paper_id,status})}); const data=await res.json().catch(()=>({})); if(!res.ok) throw new Error(data.message||data.error||'reading status update failed'); state.paper.status=data.status||data.reading_status||'Read'; state.paper.reading_status=data.reading_status||data.status||'Read'; const idx=state.papers.findIndex(p=>p.paper_id===state.paper.paper_id); if(idx>=0){ state.papers[idx].status=state.paper.status; state.papers[idx].reading_status=state.paper.reading_status; } selectPaper(state.paper); renderAiList(); setReaderStatusNotice(`Saved: ${state.paper.reading_status}`,'ok'); }catch(err){ setReaderStatusNotice(`Status save failed: ${err.message||err}`,'error'); } finally{ if(btn) btn.disabled=false; } }
...
function setupAi(){ fillFilters('ai-'); renderTaxonomy(); ['ai-paper-search','ai-topic-filter','ai-source-filter','ai-tag-filter','ai-reading-status-filter'].forEach(id=>document.getElementById(id)?.addEventListener('input',()=>{ if(id!=='ai-paper-search'){ state.statusFilter=''; state.taxonomyMode='all'; state.taxonomyValue=''; renderTaxonomy(); } renderAiList(); })); $('#show-pdf')?.addEventListener('click',()=>setReadingView('pdf')); $('#pdf-prev-page')?.addEventListener('click',()=>changePdfPage(-1)); $('#pdf-next-page')?.addEventListener('click',()=>changePdfPage(1)); $('#pdf-zoom-in')?.addEventListener('click',()=>zoomPdf(.1)); $('#pdf-zoom-out')?.addEventListener('click',()=>zoomPdf(-.1)); $('#show-note')?.addEventListener('click',()=>setReadingView('note')); $('#show-figures')?.addEventListener('click',()=>setReadingView('figures')); $('#show-audio')?.addEventListener('click',()=>setReadingView('audio')); $('#extract-current-figures')?.addEventListener('click',extractCurrentFigures); $('#send-note-to-chat')?.addEventListener('click',()=>{ const note=($('#reading-note-editor')?.value||'').trim(); const q=$('#chat-question'); if(!note||!q)return; q.value=`结合我的阅读笔记回答：${note}`; q.focus(); }); $('#save-reading-note')?.addEventListener('click',()=>{ const note=($('#reading-note-editor')?.value||'').trim(); if(!note){ addBubble('assistant','先在 Reading Notes 里写一点笔记，再保存到 Notion。'); return; } state.lastNoteDraft={question:'Reading note',answer:note,paper_key:state.paper?.paper_id}; showNoteConfirmation({destination:'AI Note',mode:'append'}); }); renderAiList(); selectInitialPaper(); }
...
function setupRanking(){ const form=$('[data-rank-form]'); if(!form) return; const input=$('[data-rank-query]'), status=$('[data-rank-status]'); form.addEventListener('submit',async e=>{ e.preventDefault(); const query=(input?.value||'').trim(); if(!query) return; if(status) status.textContent='先快速检索本地数据库；需要时再用 LLM rerank…'; try{ const res=await fetch('/api/rank',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query,provider_id:$('#chat-provider')?.value||'deepseek',model:$('#chat-model')?.value||'deepseek-v4-flash',limit:8,candidate_limit:30})}); const data=await res.json(); if(!res.ok) throw new Error(data.message||data.error||'ranking failed'); renderRankResults(data.results||[]); if(page==='library' && data.results?.length){ const ids=new Set(data.results.map(r=>r.paper_id)); const ranked=data.results.map(r=>r.paper).filter(Boolean); const rest=state.papers.filter(p=>!ids.has(p.paper_id)); state.papers=[...ranked,...rest]; renderLibrary(); } if(status){ const source=data.source==='local-index'?'本地快速检索':'LLM rerank'; const warn=data.warning?' · LLM 输出异常，已 fallback':' '; status.textContent=`${source}: Found ${data.results?.length||0} papers · ${data.model||'model'}${warn}`; } }catch(err){ if(status) status.textContent=`Error: ${err.message||err}`; } }); window.addEventListener('keydown',e=>{ if((e.metaKey||e.ctrlKey) && e.key.toLowerCase()==='k'){ e.preventDefault(); input?.focus(); } }); }

async function init(){ renderNav(); await loadPapers(); await loadModelPresets(); setupChat(); setupRanking(); if(page==='discovery'){renderMetrics();renderDiscovery();} if(page==='library'){fillFilters('');renderLibrary();['search','topic-filter','source-filter','tag-filter','reading-status-filter'].forEach(id=>document.getElementById(id)?.addEventListener('input',renderLibrary));} if(page==='ai') setupAi(); if(page==='paper') renderPaperPage(); setupAdmin(); }
