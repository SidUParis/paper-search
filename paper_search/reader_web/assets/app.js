...
function selectPaper(p){ state.paper=p; state.lastNoteDraft=null; state.selectedVisual=null; const noteEditor=$('#reading-note-editor'); if(noteEditor) noteEditor.value=''; const url=new URL(location.href); url.searchParams.set('id',p.paper_id); history.replaceState(null,'',url); $('#ai-paper-header').innerHTML=`<p class="eyebrow">${esc(p.topic_slug||'paper')} · ${esc(p.display_venue||p.venue||p.source_label||'')}</p><h1>${esc(p.title)}</h1><p>${esc((p.authors||[]).join(', '))}</p><div class="tag-row">${readingStatusBadge(p)}${(p.tags||[]).slice(0,8).map(t=>`<em>${esc(t)}</em>`).join('')}<button class="tiny mark-read-btn" data-mark-read type="button">Mark read</button><span class="reader-status-notice" data-reader-status-notice></span></div>`; $('#ai-paper-header [data-mark-read]')?.addEventListener('click',()=>updateReadingStatus('read')); const pdf=pdfUrl(p); loadPdfPreview(p); $('#open-source').href=p.source_url||pdf||'#'; $('#pdf-status').textContent=p.local_document?'local synced PDF':(pdf?'rendered same-origin PDF':'no PDF found'); $('#synced-note').innerHTML=noteHtml(p); renderFigureGallery(p); renderAudioPanel(p); const tldr=$('#insight-tldr'); if(tldr) tldr.textContent=short(summaryOf(p),260); const score=$('#insight-score'); if(score) score.textContent=scoreFor(p); const nov=$('#insight-novelty'); if(nov) nov.textContent=noveltyFor(p); const meta=$('#insight-meta'); if(meta) meta.innerHTML=`<span>${esc(metaLine(p)||'No venue')}</span><span>${esc((p.tags||[]).slice(0,2).join(' · ')||p.topic_slug||'paper')}</span>`; $$('.ai-paper-item').forEach(el=>el.classList.toggle('active',el.dataset.id===p.paper_id)); }
