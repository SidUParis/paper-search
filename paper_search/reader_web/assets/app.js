...
async function updateReadingStatus(status){ if(!state.paper){ addBubble('assistant','先选择一篇论文。'); return; } const btn=$('[data-mark-read]'); if(btn) btn.disabled=true; try{ const res=await fetch('/api/reading-status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({paper_key:state.paper.paper_id,status})}); const data=await res.json(); if(!res.ok) throw new Error(data.message||data.error||'reading status update failed'); state.paper.status=data.status||data.reading_status||'Read'; state.paper.reading_status=data.reading_status||data.status||'Read'; const idx=state.papers.findIndex(p=>p.paper_id===state.paper.paper_id); if(idx>=0){ state.papers[idx].status=state.paper.status; state.papers[idx].reading_status=state.paper.reading_status; } selectPaper(state.paper); renderAiList(); addBubble('assistant',`已标记为 ${state.paper.reading_status}，并同步写回 Notion。`); }catch(err){ addBubble('error',`Reading status update failed: ${err.message||err}`); } finally{ if(btn) btn.disabled=false; } }
