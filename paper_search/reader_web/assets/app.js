...
function renderTaxonomy(){
  ...
  const makeItem=(label,value,type,count)=>`<button class="taxonomy-item ${state.taxonomyMode===type&&state.taxonomyValue===String(value)?'active':''}" data-taxonomy-type="${type}" data-taxonomy-value="${esc(value)}"><span>${esc(label)}</span><em>${count}</em></button>`;
  if(projectBox){
...
function selectTaxonomy(type,value){
  ...
  renderTaxonomy(); renderAiList();
}
function renderTaxonomySubmenu(){
  const box=$('#taxonomy-submenu'); if(!box) return;
  if(state.taxonomyMode==='all'){ box.innerHTML='<div class="submenu-title">Choose a project, topic, or venue to focus the queue.</div>'; return; }
  const papers=filteredPapers('ai-');
  const title=taxonomyLabel(state.taxonomyMode,state.taxonomyValue);
  box.innerHTML=`<div class="submenu-title"><strong>${esc(title)}</strong><span>${papers.length} papers</span></div><div class="submenu-papers">${papers.slice(0,10).map(p=>`<button data-subpaper="${esc(p.paper_id)}"><strong>${esc(short(p.title,64))}</strong><span>${esc(metaLine(p))}</span></button>`).join('')||'<p>No papers in this group.</p>'}</div>`;
