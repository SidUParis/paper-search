const state = { papers: [], paper: null, catalog: {topics: [], sources: []}, featured: null, modelConfig: null, lastNoteDraft: null, selectedVisual: null, uploadedFiles: [], webSearch: false, viewerIndex: 0, viewerZoom: 1, pdfPage: 1, pdfPages: 0, pdfZoom: 1 };
...
function setupAi(){ fillFilters('ai-'); renderTaxonomy(); ['ai-paper-search','ai-topic-filter','ai-source-filter','ai-tag-filter'].forEach(id=>document.getElementById(id)?.addEventListener('input',()=>{ if(id!=='ai-paper-search') state.statusFilter=''; renderAiList(); })); $('#show-pdf')?.addEventListener('click',()=>setReadingView('pdf')); $('#pdf-prev-page')?.addEventListener('click',()=>changePdfPage(-1)); $('#pdf-next-page')?.addEventListener('click',()=>changePdfPage(1)); $('#pdf-zoom-in')?.addEventListener('click',()=>zoomPdf(.1)); $('#pdf-zoom-out')?.addEventListener('click',()=>zoomPdf(-.1)); $('#show-note')?.addEventListener('click',()=>setReadingView('note')); $('#show-figures')?.addEventListener('click',()=>setReadingView('figures')); $('#show-audio')?.addEventListener('click',()=>setReadingView('audio')); $('#extract-current-figures')?.addEventListener('click',extractCurrentFigures); $('#send-note-to-chat')?.addEventListener('click',()=>{ const note=($('#reading-note-editor')?.value||'').trim(); const q=$('#chat-question'); if(!note||!q)return; q.value=`结合我的阅读笔记回答：${note}`; q.focus(); }); $('#save-reading-note')?.addEventListener('click',()=>{ const note=($('#reading-note-editor')?.value||'').trim(); if(!note){ addBubble('assistant','先在 Reading Notes 里写一点笔记，再保存到 Notion。'); return; } state.lastNoteDraft={question:'Reading note',answer:note,paper_key:state.paper?.paper_id}; showNoteConfirmation({destination:'AI Note',mode:'append'}); }); renderAiList(); }
