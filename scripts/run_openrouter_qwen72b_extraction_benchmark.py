...
OUT_PATH = Path('/home/orange/paper-search/cache/openrouter_qwen72b_benchmark_results.json')
...
def openrouter_generate(prompt: str) -> tuple[str, dict[str, Any]]:
    _load_env_file('/home/orange/paper-search/.env')
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            'X-Title': 'paper-search-openrouter-qwen72b-benchmark',
        },
    )
    with urllib.request.urlopen(req, timeout=240) as resp:
        raw = resp.read().decode('utf-8', errors='replace')
    idx = raw.find('{')
    body = json.loads(raw[idx:] if idx != -1 else raw)
    message = ((body.get('choices') or [{}])[0].get('message') or {})
    return str(message.get('content') or ''), body.get('usage') or {}
...
def main() -> None:
    ...
    summary = {
        ...
        'timing': {'elapsed_seconds_total': 0.0, 'elapsed_seconds_avg': 0.0},
    ...
    for item in sample:
        ...
        try:
            started = time.time()
            text, usage = openrouter_generate(prompt)
            elapsed_seconds = time.time() - started
            raw_parsed = _extract_json_object(text)
        except Exception as exc:
            elapsed_seconds = time.time() - started if 'started' in locals() else 0.0
            parse_error = str(exc)
            text = ''

        summary['timing']['elapsed_seconds_total'] += elapsed_seconds
        ...
        summary['usage']['total_tokens'] += int(usage.get('total_tokens') or 0)
        summary['usage']['cost'] += float(usage.get('cost') or 0.0)
        ...
        print(status, project_id, item['title'][:80], f"{elapsed_seconds:.2f}s")

    summary['timing']['elapsed_seconds_avg'] = round(
        summary['timing']['elapsed_seconds_total'] / len(sample), 3
    ) if sample else 0.0
    summary['timing']['elapsed_seconds_total'] = round(summary['timing']['elapsed_seconds_total'], 3)
    summary['usage']['cost'] = round(summary['usage']['cost'], 6)
