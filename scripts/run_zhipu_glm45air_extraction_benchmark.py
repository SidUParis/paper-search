...
OUT_PATH = Path('/home/orange/paper-search/cache/zhipu_glm45air_benchmark_results.json')
...
def zhipu_generate(prompt: str) -> tuple[str, dict[str, Any]]:
    payload = json.dumps(
        {
            'model': MODEL,
            'messages': [
                {
                    'role': 'system',
                    'content': '你是一个结构化信息抽取助手，只返回最终JSON结果。',
                },
                {
                    'role': 'user',
                    'content': prompt,
...
def main() -> None:
    ...
    for item in sample:
        ...
        try:
            started = time.time()
            text, usage = zhipu_generate(prompt)
